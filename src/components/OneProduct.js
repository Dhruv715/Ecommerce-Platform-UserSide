...
function OneProduct() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`https://ecommerce-platform-kfby.onrender.com/Product/${id}`);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (response.data.status === 'Success') {
        await axios.post(`https://ecommerce-platform-kfby.onrender.com/addCart/${id}`, {
          quantity: quantity
        }, {
          headers: {
            auth: token
          ...
          <div className='p-4 flex flex-col lg:flex-row items-start'>
            <div className='flex flex-col flex-wrap justify-center mb-4 lg:mb-0'>
              {product.image.map((img, index) => (
                <img
                  key={index}
                  src={`https://ecommerce-platform-kfby.onrender.com/images/${img}`}
                  alt={product.name}
            ...
            <div className='w-full lg:w-1/2 flex justify-center mb-4'>
              <img src={`https://ecommerce-platform-kfby.onrender.com/images/${selectedImage}`} alt={product.name} className='w-96 h-96 lg:w-[40vw] lg:h-[40vw] object-contain' />
            </div>
