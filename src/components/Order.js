...
function Order() {
  ...
  useEffect(() => {
    ...
    const fetchOrderData = async () => {
      try {
        const response = await axios.get('https://ecommerce-platform-kfby.onrender.com/fetchUserOrders', {
          headers: {
            auth: token,
          },
        });
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    try {
      await axios.post(`https://ecommerce-platform-kfby.onrender.com/ChangeOrderStatus/${orderId}`, null, {
        headers: {
          auth: token,
        }
      });
