...
function Cart() {
  ...
  useEffect(() => {
    ...
    const fetchCartData = async () => {
      try {
        const response = await axios.get('https://ecommerce-platform-kfby.onrender.com/getCartData', {
          headers: {
            auth: token,
          },
        });
  ...
  const handleQuantityChange = async (index, change) => {
    ...
    if (newQuantity > 0) {
      ...
      try {
        await axios.patch(`https://ecommerce-platform-kfby.onrender.com/UpdateCart/${updatedProducts[index].productId}`, {
  ...
  const handleRemoveProduct = async (index) => {
    ...
    try {
      await axios.delete(`https://ecommerce-platform-kfby.onrender.com/DeleteProduct/${productId}`, {
        headers: {
          auth: token,
        },
      });
  ...
  const handlePlaceOrder = async () => {
    try {
      const response = await axios.post('https://ecommerce-platform-kfby.onrender.com/placeOrder', {}, {
        headers: {
          auth: token,
        },
      });
                ...
                {cartProducts.map((product, index) => (
                  <tr key={index} className="text-center">
                    <td className="py-4 px-6 border-b border-gray-200">
                      <img
                        src={`https://ecommerce-platform-kfby.onrender.com/images/${product.image}`}
                        alt={product.name}
